...
export function getUrl(path) {
  return (document.location.hostname === 'localhost') ? `/tools/mocked-apis${path}` : path;
}
...
export function getTable(block, name, path) {
  ...
  th.style.backgroundColor = 'rgb(99, 59, 61)';
...
export async function copyReplaceTable(blockVariant, tokens) {
  const { table } = blockVariant;

  // eslint-disable-next-line max-len
  const replacedTable = Object.entries(tokens).reduce((acc, [key, value]) => acc.replaceAll(key, value), table);

  return copyTable({ table: replacedTable });
}
